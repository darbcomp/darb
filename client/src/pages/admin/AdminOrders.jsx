...
function StatusBadge({ status }) {
    return (<span className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold capitalize ${statusStyles[status] ||
...
function AdminOrders() {
    ...
    const openPaymentProof = async (order) => {
        setProofError("");
        setOpeningProofOrderId(order._id);
        try {
            const response = await getAdminOrderPaymentProof(order._id);
            if (!response?.data?.url) {
                throw new Error("Payment proof URL is unavailable.");
            }
            setProofViewer({
                orderId: order._id,
    ...
    return (<section>
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-darb-muted">
            Monitor customer orders, payment proofs, payment status,
            delivery progress, and admin notes.
          </p>
        </div>
      ...
      <div className="mb-8 rounded-[1.5rem] border border-darb-gold/20 bg-white p-5 shadow-soft">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          <div className="relative md:col-span-2 xl:col-span-1">
            <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-darb-muted"/>

            <input name="search" value={filters.search} onChange={handleFilterChange} placeholder="Search order, customer, phone..." className="w-full rounded-full border border-darb-gold/30 py-3 pl-11 pr-5 outline-none transition focus:border-darb-green"/>
          </div>

          <select name="orderStatus" value={filters.orderStatus} onChange={handleFilterChange} className="rounded-full border border-darb-gold/30 bg-white px-5 py-3 capitalize outline-none transition focus:border-darb-green">
          ...
          <select name="paymentStatus" value={filters.paymentStatus} onChange={handleFilterChange} className="rounded-full border border-darb-gold/30 bg-white px-5 py-3 capitalize outline-none transition focus:border-darb-green">
            <option value="">All payment statuses</option>

            {paymentStatuses.map((status) => (<option key={status} value={status}>
                {formatStatus(status)}
              </option>))}
          </select>

          <select name="paymentMethod" value={filters.paymentMethod} onChange={handleFilterChange} className="rounded-full border border-darb-gold/30 bg-white px-5 py-3 capitalize outline-none transition focus:border-darb-green">
          ...
          <select name="paymentProofStatus" value={filters.paymentProofStatus} onChange={handleFilterChange} className="rounded-full border border-darb-gold/30 bg-white px-5 py-3 capitalize outline-none transition focus:border-darb-green">
            <option value="">All proof statuses</option>

            {paymentProofStatuses.map((status) => (<option key={status} value={status}>
                {formatStatus(status)}
              </option>))}
          </select>

          <button type="button" onClick={resetFilters} className="rounded-full border border-darb-gold/40 px-5 py-3 text-sm font-semibold text-darb-green transition hover:bg-darb-gold/15">
            Reset
          </button>
        </div>
      </div>

      {proofError && !proofViewer && (<div className="mb-6 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
          {proofError}
        </div>)}

      {ordersQuery.isLoading && (<div className="rounded-[1.5rem] border border-darb-gold/20 bg-white p-8 shadow-soft">
          <p className="text-darb-muted">Loading orders...</p>
        </div>)}

      {ordersQuery.isError && (<div className="rounded-[1.5rem] border border-red-200 bg-red-50 p-8 shadow-soft">
      ...
      {!ordersQuery.isLoading &&
            !ordersQuery.isError &&
            orders.length === 0 && (<div className="rounded-[1.5rem] border border-darb-gold/20 bg-white p-8 shadow-soft">
            ...
            <p className="mt-3 max-w-2xl leading-7 text-darb-muted">
              Orders will appear here once customers start placing Darb
              purchases.
            </p>
          </div>)}
      ...
      {!ordersQuery.isLoading &&
            ...
            {orders.map((order) => {
                ...
                const remainingItemsCount = Math.max((order.items?.length || 0) - previewItems.length, 0);
                const proofStatus = order.paymentProof?.status || "not_required";
                const hasPaymentProof = proofStatus !== "not_required";
                const proofIsSubmitted = proofStatus === "submitted";
                const proofIsRejected = proofStatus === "rejected";
                return (<article key={order._id} className="overflow-hidden rounded-[1.5rem] border border-darb-gold/20 bg-white shadow-soft">
                  <div className="grid gap-5 border-b border-darb-gold/10 p-6 xl:grid-cols-[1fr_auto]">
                    ...
                    <div className="flex flex-wrap items-start gap-3 xl:justify-end">
                      ...
                      {hasPaymentProof && (<StatusBadge status={proofStatus}/>)}
                    </div>
                  </div>
                  ...
                  <div className="grid gap-6 p-6 xl:grid-cols-[1fr_300px]">
                    <div>
                      <div className="space-y-3">
                        {previewItems.map((item, index) => (<div key={`${order._id}-${item.productSnapshot?.slug || index}`} className="flex items-center gap-4 rounded-2xl bg-darb-cream/70 p-3">
                            <div className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-darb-green">
                              {item.productSnapshot?.image ? (<img src={item.productSnapshot.image} alt={item.productSnapshot.name} className="h-full w-full object-cover"/>) : (<p className="font-display text-xs text-darb-gold">
                                  Darb
                                </p>)}
                            </div>
                        ...
                        {remainingItemsCount > 0 && (<p className="text-sm font-semibold text-darb-muted">
                      ...
                      {hasPaymentProof && (<div className="mt-5 rounded-2xl border border-darb-gold/20 bg-darb-cream/40 p-4">
                          <div className="flex flex-wrap items-start justify-between gap-4">
                            <div>
                              ...
                              {proofIsRejected &&
                            order.paymentProof?.rejectionReason && (<p className="mt-3 text-sm leading-6 text-red-700">
                                    Rejection reason:{" "}
                                    {order.paymentProof.rejectionReason}
                                  </p>)}
                            </div>

                            <button type="button" onClick={() => openPaymentProof(order)} disabled={openingProofOrderId === order._id} className="inline-flex items-center gap-2 rounded-full border border-darb-gold/40 bg-white px-4 py-2 text-sm font-semibold text-darb-green transition hover:bg-darb-gold/15 disabled:cursor-not-allowed disabled:opacity-60">
                              <Eye size={16}/>
                              {openingProofOrderId === order._id
                            ? "Opening..."
                            : "View Proof"}
                            </button>
                          </div>

                          {proofIsSubmitted && (<div className="mt-4 flex flex-wrap gap-3">
                              <button type="button" onClick={() => approvePaymentProof(order._id)} disabled={proofReviewMutation.isPending} className="inline-flex items-center gap-2 rounded-full bg-darb-green px-4 py-2 text-sm font-semibold text-darb-beige transition hover:bg-darb-black disabled:cursor-not-allowed disabled:opacity-60">
                  ...
                  {isEditing && (<div className="border-t border-darb-gold/10 bg-darb-cream/70 p-6">
                      ...
                      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                        ...
                        <div>
                          ...
                          <select name="paymentStatus" value={statusForm.paymentStatus} onChange={handleStatusFormChange} className="w-full rounded-full border border-darb-gold/30 bg-white px-5 py-3 capitalize outline-none transition focus:border-darb-green">
                            {paymentStatuses.map((status) => (<option key={status} value={status}>
                                {formatStatus(status)}
                              </option>))}
                          </select>

                          {proofIsSubmitted && (<p className="mt-2 text-xs leading-5 text-amber-700">
                              Approve the payment proof above before marking this order as paid.
                            </p>)}
                        </div>
                        ...
                        <div className="xl:col-span-2">
                          ...
                          <input name="note" value={statusForm.note} onChange={handleStatusFormChange} placeholder="Example: Order confirmed with customer" className="w-full rounded-full border border-darb-gold/30 bg-white px-5 py-3 outline-none transition focus:border-darb-green"/>
                        </div>
                        ...
                        <div className="md:col-span-2 xl:col-span-4">
                          ...
                          <textarea name="adminNotes" value={statusForm.adminNotes} onChange={handleStatusFormChange} rows={3} placeholder="Internal notes for the store team" className="w-full rounded-3xl border border-darb-gold/30 bg-white px-5 py-3 outline-none transition focus:border-darb-green"/>
                        </div>
                      </div>

                      <div className="mt-5 flex flex-wrap gap-3">
                        <button type="button" onClick={() => submitStatusUpdate(order._id)} disabled={updateMutation.isPending} className="rounded-full bg-darb-green px-6 py-3 text-sm font-semibold text-darb-beige transition hover:bg-darb-black disabled:cursor-not-allowed disabled:opacity-60">
                          {updateMutation.isPending ? "Saving..." : "Save Update"}
                        </button>

                        <button type="button" onClick={() => setEditingOrderId("")} className="rounded-full border border-darb-gold/40 px-6 py-3 text-sm font-semibold text-darb-green transition hover:bg-darb-gold/15">
                          Cancel
                        </button>
                      </div>
                    </div>)}
            ...
            <div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-darb-gold/20 bg-white p-5">
              <div>
                ...
                <p className="mt-1 text-sm text-darb-muted">
                  {proofViewer.customerName} · Secure temporary view
                </p>
              </div>
              ...
              <button type="button" onClick={() => {
                setProofViewer(null);
                setProofError("");
            }} className="rounded-full border border-darb-gold/30 p-2 text-darb-green transition hover:bg-darb-gold/15" aria-label="Close payment proof">
                <X size={20}/>
              </button>
            </div>

            <div className="p-5">
              {proofError && (<div className="mb-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                  {proofError}
                </div>)}

              <div className="overflow-hidden rounded-2xl bg-darb-cream/60 p-3">
                <img src={proofViewer.url} alt={`Payment proof for ${proofViewer.orderNumber}`} className="mx-auto max-h-[70vh] w-auto max-w-full rounded-xl object-contain"/>
              </div>

              <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                <div className="flex flex-wrap items-center gap-2">
                  <StatusBadge status={proofViewer.status}/>

                  <span className="text-xs text-darb-muted">
                    This secure link expires in about {Math.round(proofViewer.expiresInSeconds / 60)} minutes.
                  </span>
                </div>

                {proofViewer.status === "submitted" && (<div className="flex flex-wrap gap-2">
                    <button type="button" onClick={() => approvePaymentProof(proofViewer.orderId)} disabled={proofReviewMutation.isPending} className="inline-flex items-center gap-2 rounded-full bg-darb-green px-4 py-2 text-sm font-semibold text-darb-beige transition hover:bg-darb-black disabled:cursor-not-allowed disabled:opacity-60">
                      <CheckCircle2 size={16}/>
                      Approve
                    </button>
