...
const rewardSegments = ["5% off", "20% Musk", "Free shipping", "Free extra tester", "10% after 1 order"];
...
export default function SpinWheel({ onClose }) {
  ...
  useEffect(() => {
    ...
    const documentStyle = document.documentElement.getAttribute("style");
    ...
    return () => {
      ...
      if (documentStyle === null) document.documentElement.removeAttribute("style");
      else document.documentElement.setAttribute("style", documentStyle);
  ...
  const guestMutation = useMutation({
    mutationFn: () => claimGuestOrderSpin({ orderNumber: orderNumber.trim() ? `DARB-${orderNumber.trim()}` : "", phone: phone.trim() }),
    onSuccess: (response) => {
    ...
    <section ref={dialogRef} tabIndex={-1} className="spin-wheel-dialog relative my-auto w-full max-w-2xl overflow-hidden rounded-[2rem] border border-darb-gold/35 bg-darb-cream p-6 text-center shadow-2xl outline-none sm:p-9" role="dialog" aria-modal="true" aria-labelledby="reward-wheel-title">
      ...
      {isAuthenticated ? <div className="mt-6">
        {rewardsQuery.isSuccess && availableSpins.length === 0 ? <div className="rounded-[1.5rem] border border-darb-gold/30 bg-darb-surface px-5 py-6 text-center sm:px-7" role="status">
          ...
          <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-darb-muted">{t("You’ll earn a new spin after an eligible order is delivered.")}</p>
        </div> : <>
      ...
      </div> : <div className="mt-7 rounded-[1.5rem] bg-darb-surface p-5 text-start sm:p-6"><div><h3 className="font-display text-2xl text-darb-green">{t("New to Darb?")}</h3><p className="mt-2 text-sm leading-6 text-darb-muted">{t("Create an account and get your first spin.")}</p><button type="button" onClick={goToRegistration} className="mt-4 min-h-12 w-full rounded-full bg-darb-green px-6 font-semibold text-darb-beige">{t("Sign up & get a spin")}</button></div><div className="my-5 flex items-center gap-3" aria-hidden="true"><span className="h-px flex-1 bg-darb-gold/30"/><span className="text-xs font-semibold uppercase tracking-[0.2em] text-darb-muted">{t("or")}</span><span className="h-px flex-1 bg-darb-gold/30"/></div><div><h3 className="font-display text-2xl text-darb-green">{t("Ordered as a guest?")}</h3><p className="mt-2 text-sm leading-6 text-darb-muted">{t("Use the order number and phone number from a delivered Darb order.")}</p><label htmlFor="guest-spin-order" className="mt-4 block text-xs font-semibold text-darb-green">{t("Order number")}</label><div dir="ltr" onMouseDown={(event) => { if (event.target === orderNumberInputRef.current) return; event.preventDefault(); orderNumberInputRef.current?.focus(); }} className="mt-2 flex cursor-text overflow-hidden rounded-full border border-darb-gold/35 bg-darb-cream transition focus-within:border-darb-green focus-within:ring-1 focus-within:ring-darb-green"><span className="flex cursor-text items-center border-e border-darb-gold/25 bg-darb-surface px-5 text-sm font-bold tracking-wide text-darb-green" aria-hidden="true">DARB-</span><input ref={orderNumberInputRef} id="guest-spin-order" value={orderNumber} onChange={(event) => setOrderNumber(event.target.value.replace(/^\s*DARB\s*-?\s*/i, "").replace(/\D/g, ""))} placeholder="1002" inputMode="numeric" pattern="[0-9]*" autoComplete="off" aria-label={t("Order number digits")} className="min-w-0 flex-1 border-0 bg-transparent px-4 py-3 outline-none focus:outline-none focus:ring-0" /></div><label htmlFor="guest-spin-phone" className="mt-4 block text-xs font-semibold text-darb-green">{t("Phone used for your order")}</label><input id="guest-spin-phone" value={phone} onChange={(event) => setPhone(event.target.value)} inputMode="tel" autoComplete="tel" placeholder="01XXXXXXXXX" className="mt-2 w-full rounded-full border border-darb-gold/35 bg-darb-cream px-5 py-3 outline-none" /><button type="button" onClick={() => { resetResult(); guestMutation.mutate(); }} disabled={pending || !/^\d+$/.test(orderNumber.trim()) || phone.trim().length < 11 || Boolean(result)} className="mt-4 min-h-12 w-full rounded-full bg-darb-green px-6 font-semibold text-darb-beige disabled:cursor-not-allowed disabled:opacity-50">{t(pending ? "Checking your order…" : result ? "Reward revealed" : "Claim order spin")}</button>{result && <button type="button" onClick={resetResult} className="mt-3 w-full text-center text-sm font-semibold text-darb-green underline decoration-darb-gold underline-offset-4">{t("Claim another delivered-order spin")}</button>}{guestRewards.length > 0 && <p className="mt-4 text-xs text-darb-muted">{t("Your latest guest reward is kept on this device so its code survives a refresh.")}</p>}</div></div>}
