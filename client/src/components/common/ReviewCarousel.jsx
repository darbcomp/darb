...
export function ReviewCard({ review }) {
  ...
  return <article className="flex w-full shrink-0 snap-start flex-col overflow-hidden rounded-[1.5rem] border border-darb-gold/25 bg-darb-surface p-5 sm:w-[calc(50%_-_0.5rem)] lg:w-[calc(25%_-_0.75rem)]">
    ...
    <p dir="auto" className="mt-5 flex-1 text-sm leading-7 text-darb-black/80">{review.text}</p>
    <div className="mt-6 border-t border-darb-gold/20 pt-4">
      <div className="flex items-center gap-2"><p dir="auto" className="font-semibold text-darb-green">{review.displayName}</p>{review.isVerifiedPurchase && <BadgeCheck size={16} className="text-darb-green" aria-label={t("Verified Purchase")} />}</div>
